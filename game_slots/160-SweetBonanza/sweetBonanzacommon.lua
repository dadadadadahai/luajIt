...
function Normal(gameId,gameType, betindex, datainfo, datainfos, uid)
    ...
    BackpackMgr.GetRewardGood(datainfos._id, Const.GOODS_ID.GOLD,winScore+result.jackpotChips, Const.GOODS_SOURCE_TYPE.SWEETBONANZA)
    unilight.update(Table, datainfos._id, datainfos)
    local boards={}
...
function CommonRotate(uid,gameId, gameType,isfree,datainfo)
    ...
    -- local chessdata, lastColRow = gamecommon.CreateSpecialChessData(cols, table_160_normalspin)

    local cfg,rtp = gamecommon.GetSpin(uid,GameId,gameType)
    if table.empty(datainfo.free)==false then
        cfg = sweetBonanza['table_160_free_'..gameType]
    ...
    -- 判断出不出倍数图标
    local mulcfg={}
    local table_num = sweetBonanza['table_mul_gamety'..gameType]
    local table_mul = sweetBonanza['table_160_mul_'..gameType]
    local num = 0
    local fieldName=''
...
function EliminateProcess(chessdata, lastColRow, disInfo, chip, uid, gameType,cfg,EliminateInfo)
    ...
    for key, val in pairs(statMap) do
        if table_160_paytable[key] ~= nil then
            local paytablekey = table_160_paytable[key]
    ...
    if bl then
        ...
        -- gamecommon.FillZeroChess(chessdata, lastColRow, table_160_normalspin)
