...
function CommonRotate(uid,gameId, gameType,isfree,datainfo)
    ...
    -- local chessdata, lastColRow = gamecommon.CreateSpecialChessData(cols, table_121_normalspin)

    local cfg,rtp = gamecommon.GetSpin(uid,GameId,gameType)
    if table.empty(datainfo.free)==false then
        cfg = cleopatraNew['table_121_free_'..gameType]
    ...
    -- 判断出不出倍数图标
    local mulcfg={}
    local table_num = cleopatraNew['table_mul_gamety'..gameType]
    local table_mul = cleopatraNew['table_121_mul_'..gameType]
    local num = 0
    local fieldName=''
...
function EliminateProcess(chessdata, lastColRow, disInfo, chip, uid, gameType,cfg,EliminateInfo)
    ...
    for key, val in pairs(statMap) do
        if table_121_paytable[key] ~= nil then
            local paytablekey = table_121_paytable[key]
    ...
    if bl then
        ...
        -- gamecommon.FillZeroChess(chessdata, lastColRow, table_121_normalspin)
