...
function addfakeres2(cboards,disInfo,isfake)
	...
	while true do
		...
		local diswl = gamecommon.WiningLineFinalCalc(chessdata,table_126_payline,table_126_paytable,wilds,nowild)
		local winLines = {}
		local  winMul = 0
		-- 计算中奖线金额
		for k, v in ipairs(diswl) do
...
function  addfakeres345(boards,disInfo)
	local isfake = nil 
	local wnum = calc_W() --渔夫
	local unum = calc_dayu()--大雨
