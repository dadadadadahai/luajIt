...
local chip = gamecommon.GetBetConfig(1,corpse.LineNum)[1]
local tChip = chip * corpse.LineNum
...
local bonusMap={}
bonusMap['bonus1']={triggerNum=0,winScore =0}
bonusMap['bonus2']={triggerNum=0,winScore =0}
bonusMap['bonus3']={triggerNum=0,winScore =0}
bonusMap['bonus4']={triggerNum=0,winScore =0}
...
function BackFunction(data)
    ...
    if table.empty(features.bonus)==false then
        local bonusName = 'bonus'..features.bonus.extraData.type
        if status~=Const_status.Bonus then
            ...
            bonusMap[bonusName] = bonusMap[bonusName] or {triggerNum = 0,winScore=0}
            bonusMap[bonusName].triggerNum = bonusMap[bonusName].triggerNum + 1
        end
        local bonus = features.bonus
        if bonus.lackTimes<=0 then
            --free结束
            status =lastStatus
            bonusMap[bonusName].winScore = bonusMap[bonusName].winScore + bonus.tWinScore
        end
...
function OverShow()
    ...
    for key, value in pairs(bonusMap) do
        print(key..':trigger='..value.triggerNum..',winScore='..value.winScore)
    end
    print('-------------------------------------------------')
