...
local chip = gamecommon.GetBetConfig(1,1)[1]
local tChip = chip * 1
...
function BackFunction(data)
    ...
    if table.empty(features.bonus)==false then
        ...
        if bonus.lackTimes<=0 and bonus.bonusType == 2 then
            --free结束
            status =lastStatus
...
function StatisticswinMul(data)
    ...
    elseif table.empty(features.bonus)==false and features.bonus.lackTimes<=0 then
        ...
        -- end
        if features.bonus.bonusType == 2 then
            playNum = playNum + 1
            tWinScore = tWinScore + features.bonus.tWinScore
            --计算倍数
...
function StatisticsIcons(data)
    iconmaps[data.boards[1][1]] = iconmaps[data.boards[1][1]] or 0
    iconmaps[data.boards[1][1]] = iconmaps[data.boards[1][1]] + 1
end
