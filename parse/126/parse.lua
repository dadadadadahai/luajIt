...
function BackFunction(data)
    ...
    if table.empty(features.respin)==false then
        if status~=Const_status.Respin then
            ...
            sessionMap[Const_status.Normal].winScoreB = sessionMap[Const_status.Normal].winScoreB + data.winScoreB
            lastStatus=status
    ...
    elseif table.empty(features.free)==false then
        ...
        if status~=Const_status.Free then
            ...
            sessionMap[Const_status.Free] = sessionMap[Const_status.Free] or {triggerNum=0,winScore =0,winScoreB = 0,wildNum = 0}
            sessionMap[Const_status.Free].triggerNum = sessionMap[Const_status.Free].triggerNum + 1
        end
        local free = features.free
        sessionMap[Const_status.Free].winScoreB = sessionMap[Const_status.Free].winScoreB + data.winScoreB
        sessionMap[Const_status.Free].wildNum = sessionMap[Const_status.Free].wildNum + data.wildNum
...
function StatisticswinMul(data)
    ...
    elseif table.empty(features.free)==false and features.free.lackTimes<=0 then
        ...
        freeWinScores[features.free.tWinScore] = freeWinScores[features.free.tWinScore] or 0
        freeWinScores[features.free.tWinScore] = freeWinScores[features.free.tWinScore] + 1
        tWinScore = 0
...
function OverShow()
    ...
    print('Normal:'..',winScoreB='..sessionMap[Const_status.Normal].winScoreB)
    print('Free:'..',winScoreB='..sessionMap[Const_status.Free].winScoreB..',wildNum='..sessionMap[Const_status.Free].wildNum)
