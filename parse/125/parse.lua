...
sessionMap[2] = {triggerNum=0,winScore =0}
sessionMap[3] = {triggerNum=0,winScore =0}
sessionMap[4] = {triggerNum=0,winScore =0}
sessionMap[5] = {triggerNum=0,winScore =0}
sessionMap[6] = {triggerNum=0,winScore =0}
function BackFunction(data)
    local winScore =0
    local tchip = data.payScore
    if table.empty(data.features.bonus)==false and data.features.bonus.type~=0 then
        local type  = data.features.bonus.type + 1
        sessionMap[type].triggerNum = sessionMap[type].triggerNum + 1
        sessionMap[type].winScore = sessionMap[type].winScore + data.winScore
        if data.features.bonus.lackTimes<=0 then
            winScore = data.features.bonus.tWinScore
            TriggerNormalScore = 0
            winMul[winScore/tchip] = winMul[winScore/tchip] or 0
            winMul[winScore/tchip] = winMul[winScore/tchip] + 1
        end
        status = Const_status.Bonus
        disInfoAna(data)
    elseif table.empty(data.features.bonus)==false and data.features.bonus.type==0 then
        --触发时
        winScore = data.winScore
        TriggerNormalScore = data.winScore
