...
local chip = gamecommon.GetBetConfig(1,happypumpkin.LineNum)[1]
local tChip = chip * happypumpkin.LineNum
...
sessionMap[Const_status.Free]={triggerNum=0,winScore =0,oneTriggerNum=0,twoTriggerNum=0}
...
function BackFunction(data)
    ...
    elseif table.empty(features.free)==false then
        ...
        if status~=Const_status.Free then
            ...
            if data.extraData.freeTriggerType==1 then
                --收集触发
                sessionMap[Const_status.Free].oneTriggerNum = sessionMap[Const_status.Free].oneTriggerNum + 1
            else
                --图标触发
                sessionMap[Const_status.Free].twoTriggerNum = sessionMap[Const_status.Free].twoTriggerNum + 1
            end
        end
...
function OverShow()
    ...
    print('Free:trigger='..sessionMap[Const_status.Free].triggerNum..',collectTrigger='..sessionMap[Const_status.Free].oneTriggerNum..',iconTrigger='..sessionMap[Const_status.Free].twoTriggerNum..',winScore='..sessionMap[Const_status.Free].winScore)
