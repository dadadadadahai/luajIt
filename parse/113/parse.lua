...
local chip = gamecommon.GetBetConfig(1,30)[1]
local tChip = chip * 30
...
function BackFunction(data)
    ...
    elseif table.empty(features.free)==false then
        ...
        if status~=Const_status.Free then
            if features.free.totalTimes == 0 then
                StatisticsIcons(data)
                sessionMap[Const_status.Normal].triggerNum = sessionMap[Const_status.Normal].triggerNum + 1
                sessionMap[Const_status.Normal].winScore = sessionMap[Const_status.Normal].winScore + data.winScore
        ...
        if free.lackTimes<=0 and free.totalTimes > 0 then
            --free结束
...
function StatisticswinMul(data)
    local features = data.features
    if status==Const_status.Normal and((table.empty(features.free)==false and features.free.totalTimes == 0) or table.empty(features.bonus)==false) then
...
function StatisticsIcons(data)
    if table.empty(data.winMuls) then
        return
    end
    local icons = {}
    for _, value in ipairs(data.winMuls) do
        local iconId = value.ele
        iconmaps[iconId] = iconmaps[iconId] or {}
        iconmaps[iconId][value.num] = iconmaps[iconId][value.num] or 0
        iconmaps[iconId][value.num] = iconmaps[iconId][value.num] + 1
    end
end
-- function StatisticsIcons(data)
--     if table.empty(data.winPoints) then
--         return
--     end
--     local icons = {}
--     for _, value in ipairs(data.winPoints[1]) do
--         local iconId = data.boards[1][value[1]][value[2]]
--         icons[iconId] = icons[iconId] or 0
--         icons[iconId] = icons[iconId] + 1
--     end
--     for iconId, value in pairs(icons) do
--         iconmaps[iconId] = iconmaps[iconId] or {}
--         iconmaps[iconId][value] = iconmaps[iconId][value] or 0
--         iconmaps[iconId][value] = iconmaps[iconId][value] + 1
...
function OverShow()
    ...
    -- print('Num50 = '..FiveDragons.Num50)
    -- print('Num100 = '..FiveDragons.Num100)
    -- print('')
    -- print('Rtp50 = '..FiveDragons.Score50)
    -- print('Rtp100 = '..FiveDragons.Score100)
