...
local chip = gamecommon.GetBetConfig(GameType,beautydisco.LineNum)[1]
local tChip = chip * beautydisco.LineNum
...
sessionMap[Const_status.Bonus]={triggerNum=0,winScore =0,turnnum=0}     --转动次数
function BackFunction(data)
    ...
    if table.empty(features.bonus)==false then
        if status~=Const_status.Bonus then
            ...
            sessionMap[Const_status.Bonus].turnnum = sessionMap[Const_status.Bonus].turnnum + 1
        end
...
function OverShow()
    ...
    print('Bonus:trigger='..sessionMap[Const_status.Bonus].triggerNum..',winScore='..sessionMap[Const_status.Bonus].winScore..',turnnum='..sessionMap[Const_status.Bonus].turnnum)
    print('-------------------------------------------------')
	...
	print('tval='..tval..',tmulwinscore='..tMul*chip*beautydisco.LineNum)
end
