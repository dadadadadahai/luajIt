...
function GamePoolInit3(gameId)
    ...
    for gameType, chipsInfo in pairs(poolConfigs.chipsConfigs) do
        GAME_POOL_CONFIG3[gameId] = GAME_POOL_CONFIG3[gameId] or {}
        ...
        gamePool.gameTypeInfos[gameType] = {
            ...
            maxBetChips = chipsInfo.maxBetChips or 0,    --最大下注金额
            bombTime      = 0,                      --假奖池爆池时间
            bombChips     = 0,                      --假奖池爆池金额
...
function ReqGamePoolBet3(gameId, gameType, chips)
    local gamePool     = GAME_POOL_CONFIG3[gameId]
...
function GamePoolsTick3()
    ...
    -- end
    --增加假奖池金额
    for gameId, gamePool in pairs(GAME_POOL_CONFIG3) do
        -- 判断是否超出周期
        if os.time() >= gamePool.lastTimeSec + gamePool.intervalSec then
            if gamePool.poolType == 5 then
            ...
            if gamePool.poolType == 5 then
                for gameType, gameTypeInfo in pairs(gamePool.gameTypeInfos) do
                    ...
                    if bBomb then
                        ...
                        --假奖池暴出金额
                        -- local gradeConfigs = gamePool.gameTypeInfos[gameType].poolConfigs.gradeConfigs
                        local gradeConfig = import ("table/game/"..gameId.."/table_"..gameId.."_jackpotgrade")[gameType]
                        local addScore = math.random(gradeConfig.min,gradeConfig.max)
                        local bombChips = math.floor(gameTypeInfo.fakePoolChips * (fakeScaleConfig.fakePoolPer / 100)) + addScore
                        gameTypeInfo.bombChips = bombChips
                        --如果真奖池的金额小于这个金额,未来随机时间为假奖池减掉这个金额
                        -- if gameTypeInfo.realPoolChips < bombChips then
                        -- 最大爆池倍数
                        local maxChips = scaleConfigs[#scaleConfigs].mul * gamePool.gameTypeInfos[gameType].maxBetChips
                        if gameTypeInfo.realPoolChips < math.floor(gameTypeInfo.fakePoolChips * (scaleConfigs[#scaleConfigs].fakePoolPer / 100)) + maxChips then
...
function GetGamePoolChips3(gameId, gameType, betIndex)
    local gamePool = GAME_POOL_CONFIG3[gameId]
    local gameTypeInfo = gamePool.gameTypeInfos[gameType]
    local jackpotNum = 0
    ...
    if os.time() > gameTypeInfo.realEndTime then
        return false, jackpotNum, jackpotChips 
    end
    ...
    -- 爆池倍数
    local jackpotMul = realScaleConfig.mul
    --可以爆池了
    jackpotNum = gameTypeInfo.jackpotNum
    ...
    return true, jackpotNum, jackpotChips, jackpotMul
end

-- 业务层主动调用减少奖池金额
function ReducePoolChips(gameId,gameType,jackpotChips)
    local gamePool = GAME_POOL_CONFIG3[gameId]
    local gameTypeInfo = gamePool.gameTypeInfos[gameType]
    -- 真奖池爆池减少假奖池金额
...
desc : 推送奖池配置
--]]
function SendGamePoolBase3(uid)
    --没在游戏中不发
...
desc : gm获得奖池配置
--]]
function GetPoolConfig3(gameId, gameType)
    local gamePool = GAME_POOL_CONFIG3[gameId]
    if gamePool == nil then
        local poolConfig = {
            subgameid         = gameId, 
            ...
            fakepoolmax    = 0,
            limitlow       = 0,
            rebatevalue    = gamecommon.GetGameRTP(gameId, gameType), --todo等待填充,
            standardchips  = 0,
            bomblooptime   = 0,
    ...
    local poolConfig = {
        ...
        rebatevalue    = gamecommon.GetGameRTP(gameId, gameType),
...
--]]
function SetPoolConfig3(gameId, gameType, poolConfig)
    ...
    if GAME_POOL_CONFIG3[gameId] == nil then
        unilight.info(string.format("gm设置slots奖池,游戏ID:%d, 游戏场次:%d, 没有奖池数据", gameId, gameType))
        return
    end
    local gamePool = GAME_POOL_CONFIG3[gameId]
    local gameTypeInfo = GAME_POOL_CONFIG3[gameId].gameTypeInfos[gameType]
    local addPerConfig  = gamePool.poolConfigs.addPerConfigs[1]
    -- local gameTypeInfo = gamePool.gameTypeInfos[gameType]
    -- local addRealPoolPer = gamePool.poolConfigs.addPerConfigs[1].addRealPoolPer
...
function GetJackpotIconNum3(gameId, gameType)
    local gamePool = GAME_POOL_CONFIG3[gameId]
    local gameTypeInfo = gamePool.gameTypeInfos[gameType]
...
function JackpotClocker3()
    -- if os.time() == GAME_POOL_CONFIG3[111].gameTypeInfos[1].realEndTime then
    --     local msg = {
    --         betIndex = 5,
    --         gameType = 1,
    --     }
